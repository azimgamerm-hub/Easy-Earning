...
const JobApplyModal = ({ job, onClose }: { job: Job, onClose: () => void }) => {
    ...
    const handleSubmit = () => {
        ...
        if (!isAllProofsSubmitted) {
            setMessage('অনুগ্রহ করে সমস্ত প্রয়োজনীয় প্রমাণ দিন।');
            return;
        }
        ...
        setMessage('আপনার প্রমাণ সফলভাবে জমা দেওয়া হয়েছে!');
        setTimeout(() => {
            onClose();
        }, 2000);
    };
                ...
                <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
                    ...
                    {job.rules && (
                        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                            <h4 className="font-bold text-lg text-gray-800 mb-2 text-center">Rule Box</h4>
                            <div className="text-gray-700 whitespace-pre-wrap text-sm">{job.rules}</div>
                        </div>
                    )}

                    <div className="border-t pt-4">
                      <h4 className="font-semibold text-gray-700 mb-2">আপনার প্রমাণ জমা দিন</h4>
                 ...
                 {message && <p className={`text-center text-sm px-6 py-2 ${message.includes('সফলভাবে') ? 'text-green-600' : 'text-red-600'}`}>{message}</p>}
                <div className="p-4 border-t flex justify-end space-x-3 bg-gray-50 rounded-b-lg">
                    <button onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">বাতিল করুন</button>
